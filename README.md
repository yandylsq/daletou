...
[![Version](https://img.shields.io/badge/version-v12.0-blue.svg)](https://github.com/yourusername/daletou)
[![Python](https://img.shields.io/badge/python-3.8+-green.svg)](https://www.python.org/)
[![License](https://img.shields.io/badge/license-MIT-orange.svg)](LICENSE)

## 📋 系统简介

基于**多维度统计分析**、**机器学习模型融合**和**智能采样评分**的大乐透智能预测系统。系统集成了 Stacking 元学习、LSTM 时序预测、网页参考分析等多种先进算法，提供科学的号码组合推荐服务。

⚠️ **重要声明**：本系统仅供娱乐和学习用途，彩票开奖为随机事件，任何预测结果不构成投注建议。请理性购彩。

**🆕 V12数据驱动版**：基于2829期历史统计的严谨评分系统，修正过度拟合问题！
...
- ✅ **历史数据支撑**：基于2829期真实数据统计，而非单一案例
- ✅ **和值评分修正**：85-115区+380分（历史48%最高频），50-75区+250分（20.43%）
- ✅ **小号评分优化**：2个小号+210分（37.22%最高频），3个小号+100分（15.20%）
- ✅ **连号评分优化**：1对连号+120分（39.79%最常见），3对连号-100分（0.81%较少见）
- ✅ **质数评分优化**：2个质数+120分（36.04%最常见），避免盲目加分
- ✅ **012路评分优化**：聚集+120分（60.78%出现率），合理设定权重
- ✅ **AC值评分优化**：AC值5-6+180分（67.92%最常见），符合数据分布
- ✅ **趋势动态调整**：结合最近100期趋势进行±20%微调
- ✅ **性能优化**：特征缓存+快速过滤，5+2速度提升3-4倍，8+3提升数十倍
- ✅ **全量遍历**：不采样、不跳过，保证100%覆盖率，不遗漏任何组合

### 🛡️ 过滤规则与模块独立性

#### 📊 系统模块划分

本项目分为**三个独立模块**，过滤规则完全独立：

1、**历史开奖数据模块**
   - 直接读取 `daletou_history_full.txt` 文件
   - **无任何过滤条件**
...
3、**号码组合导出与预测模块**
   - 包含三个功能：导出、开始预测、号码组合查询
   - **仅此模块有过滤条件**，且三个功能共享过滤规则
...
红球前置过滤：
- ❌ 全奇全偶（5个球全为奇数或全为偶数）
- ❌ 四连号及以上（如 01-02-03-04 或更长连号）
- ❌ 等差数列（如 03-06-09-12-15，公差相同）
- ❌ 等比数列（如 02-04-08-16，公比相同）
- ❌ 同区号码（5个球全在01-11区、12-23区或24-35区）

完整组合前置过滤：
- ❌ 历史已开奖号码（与历史任意一期完全相同的5+2组合）
...
- 🎯 和值范围（前区5个红球和值区间）
- 🎯 奇偶比（前区红球奇偶数量比例）
- 🎯 重号限制（与上期红球重复>=4个或蓝球重复>=2个）

**评分与排序**：通过前两级过滤的组合，进入基于历史统计的多维度评分系统，最终按评分降序输出Top N。

### 🎯 技术亮点

- ✅ **特征缓存评分**：预计算红球特征缓存，避免重复计算，全量遍历不遗漏
